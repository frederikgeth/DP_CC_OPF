function run_mechanism_nlp(mechanism,node,line,σ,Σ,T,U_n,D_n,U_l,D_l,η_g,η_u,η_f,ψ,σ̂,θ,ϱ; optimizer=Ipopt.Optimizer)
    ...
    for i in PV_bus
        ...
        @constraint(m, sum(arg_p.^2) <= (node[i].p̅ - g_p[i])^2)
        @constraint(m, sum(arg_q.^2) <= (node[i].q̅ - g_q[i])^2)
        @constraint(m, sum(arg_p.^2) <= (g_p[i])^2)
        @constraint(m, sum(arg_q.^2) <= (g_q[i])^2)
    ...
    for i in L
        ...
        @constraint(m, sum(arg.^2) <= (node[i].v̅ - u[i])^2)
        @constraint(m, sum(arg.^2) <= (u[i] - node[i].v̲)^2)
    ...
    for i in L, c in C
        ...
        @constraint(m, sum(arg.^2) <= (- α_f[c]*f_p[i] - β_f[c]*f_q[i] - δ_f[c]*line[i].f̅)^2)
